...
function bashDescription(background: boolean, promote: boolean, escalationModes: readonly SandboxMode[]): string {
  const base = 'Execute a bash command (`bash -c`) and return its stdout/stderr. '
    ...
    + (background ? 'Background jobs return a job id; collect with `job_output` and stop with `job_kill`. ' : '')
    + (promote ? 'A foreground command that reaches its timeout is not killed: it moves to a background job.' : '')
  if (escalationModes.length === 0) return base.trimEnd()
  return base.trimEnd() + ' Attempting a command the sandbox may deny is safe and expected: run it and read the '
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  const bashTool = (jobs: JobRegistry | undefined): ToolDefinition => {
    ...
    return defineTool({
      name: 'bash',
      description: bashDescription(background, promote, escalationModes),
      parameters: {
        ...
        description: {
          ...
          description: 'Clear, concise description of what this command does in active voice, '
            + '5-10 words (shown in the UI). Use the language of the user\'s latest request unless the user explicitly asks for another language. Examples: "ls" → "List files in current directory"; '
        ...
        ...escalationModes.length > 0 ? {
          ...
          justification: {
            type: 'string' as const,
            description: 'Required with sandbox_permissions and shown to the user before approval: use the language of the user\'s latest request unless the user explicitly asks for another language, and explain in plain language what this exact command will do, what files, data, or services it may affect, its main risk, and whether its changes are reversible.',
          },
        } : {},
      },
      output: {
