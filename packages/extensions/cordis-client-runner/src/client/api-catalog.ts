...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'register<N extends Extract<keyof LocaleNamespaceMap, string>>( ns: N, dicts: Record<BuiltInLocaleId, LocaleDictOf<N>> & Readonly<Record<string, LocaleDictOf<N>>>, ): () => void',
        description: 'Register a declared namespace\'s dictionaries, all locales in one call — the typed form: each dictionary is checked against the namespace\'s LocaleNamespaceMap key union (a missing or extra key is a compile error). Every built-in locale is required; a language pack may add complete dictionaries for additional BCP 47 locale ids in the same atomic registration. Duplicate (ns, locale) throws (single occupant; a namespace\'s texts have one owner). Registration bumps the revision so mounted outlets pick up late-arriving dictionaries.',
        parameters: [{ name: 'ns', description: 'a namespace merged into LocaleNamespaceMap.' }, { name: 'dicts', description: 'complete dictionaries keyed by built-in and optional language-pack locale ids.' }],
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'ChatBackground',
    declaration: 'export interface ChatBackground {\n    id: ChatBackgroundId;\n    url?: string;\n    layout?: ChatBackgroundLayout;\n}',
  },
  {
    name: 'ChatBackgroundId',
    declaration: 'export type ChatBackgroundId = typeof CHAT_BACKGROUND_IDS[number];',
  },
  {
    name: 'ChatBackgroundLayout',
    declaration: 'export type ChatBackgroundLayout = \'focus-left\' | \'focus-right\';',
  },
  {
    name: 'ChildrenDecl',
