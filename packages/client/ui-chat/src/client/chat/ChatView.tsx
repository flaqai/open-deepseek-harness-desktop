...
export function ChatView({
  ...
  sessionId, openFile, revealFile, openSkill, openExternalLink, loadOlder, loadThrough,
  loadImage, inspectCall, chatScroll, forkAt, fileMentions,
  ...
  const [fileOpenError, setFileOpenError] = useState<{ path: string; action: 'open' | 'reveal'; message: string } | null>(null)
  ...
  const requestRevealFile = useCallback((path: string) => {
    ...
    void revealFile(path).then(
      ...
      (error: unknown) => {
        if (id !== fileOpenRequest.current) return
        setFileOpenError({ path, action: 'reveal', message: openFailureMessage(error, t('fileOpen.unknown')) })
        setFileOpenBusy(false)
      },
    )
  }, [revealFile, t])
