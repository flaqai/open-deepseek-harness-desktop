...
Trajectory 视图打开后会在时间线上方展示**关键步骤记录**条。它会声明数据来自 Session 日志，并汇总已加载的轮次、工具调用、失败操作以及执行中／已记录状态，用户可以先确认智能体实际做过什么，再查看逐条事件。
...
在 rc.8 中，取消定稿记录包括持久化前缀、只能从分片恢复的打断前缀，以及被打断的工具记录。
