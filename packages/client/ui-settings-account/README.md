---
description: "Desktop Settings provides a Connect to DeepSeek page for login, support, balance, and sign-out; the sidebar opens Settings directly."
...
Open Settings directly from the sidebar, then use Connect to DeepSeek to sign in, contact support, inspect balances, or sign out. Desktop users also receive a resumable introduction to account credit and presentation preferences.
...
Connect to DeepSeek displays a 32px circular profile image, with the account icon as fallback when the URL is absent or the image fails to load. The sidebar shows a Settings button in both account states; its collapsed form is 36 × 36 px.
...
The sidebar Settings button opens the settings panel directly and exposes the effective keyboard shortcut supplied by the shell. Connect to DeepSeek remains in the settings navigation before and after login. Its signed-out view offers Contact us and Sign in; its signed-in view offers Feedback and a confirmed Sign out.
...
Connect to DeepSeek appears first in Settings regardless of login state. The plugin owns one Host snapshot stream shared through framework hooks by settings.section and settings.launcher. The launcher opens Settings directly; the section owns contact, sign-in, and confirmed sign-out actions. A failed initial account stream does not disable sign-in or retry. It maintains no independent credential state, so no invariant companion is published.
...
Account login uses a dismissible dialog opened from Desktop onboarding or Connect to DeepSeek. The dialog uses a compact, right-aligned action row with the primary action last. The account card displays the profile name, falling back to the server-masked phone number or email when the name is absent; it stays blank while the profile is loading. Dialog copy is not text-selectable; the copy-link button remains available and restores its label two seconds after each successful copy. Clipboard failures show Copy failed on the link for two seconds without interrupting sign-in. Waiting shows a copyable authorization link and a loading indicator; timeout and failure require an explicit retry, including when the account stream has no snapshot. The dialog's Copy sign-in link and the Settings Open link append the resolved Desktop palette as `theme=light` or `theme=dark` from the live theme snapshot, so both routes reach the Platform page in the current application theme without changing the theme preference Platform has saved. Sign-in failures render in that dialog; a failed sign-out keeps the confirmation dialog open for retry. Closing a waiting dialog cancels its Host attempt. The login dialog can reopen the same API-key editor through the settings coordinator.
