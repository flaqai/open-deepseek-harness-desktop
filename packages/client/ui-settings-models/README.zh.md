...
模型设置与产品引导插件。同一个 client Cordis 插件会注册 Models 页面和一套版本化的首次使用设置向导。总览提供三项任务——连接模型 API Key、通过预装的 `@xmanrui/dsh-im` 插件配置微信／飞书，以及前往“外部工具”联网安装并连接官方 Codex Provider——并把每项任务分别引导到已有的 Models 页面、Plugins／IM 页面或“外部工具”页面，而不重复绘制表单。Models 平面把三个协议领域汇聚为一个共享快照：`llm.providers`（可配置提供方目录，含每条路由的存活／休眠状态）、`settings.describe`（序列化 schema、分层脱敏值、secret slot）与 `credentials.describe`（不含值的 configured/source/writable 徽标）；页面据此渲染提供方行，一次只展开一张编辑卡片，且不把路由存活状态呈现为提供方状态。
...
向导版本位于 `src/onboarding-copy.ts`。在回环访问中，最终的“开始体验”操作会通过既有 settings API 写入 `ui-onboarding.welcomeNoticeVersion`；在此之前，任务完成与跳过选择只存在于当前打开的向导中。非回环浏览器无法使用该 Host-only namespace，因此确认只在进程内有效，刷新后仍会再次显示向导。总览右上角的语言菜单会驱动共享 Locale 服务，因此中英文切换会立即作用于向导与复用的设置页面；回环访问沿用同一 Host 偏好持久化，而且切换语言不会重新挂载向导或清空当前任务结果。只要同一份 Models 汇聚快照中已有可用提供方，模型任务便自动显示完成；否则“配置”会在引导专用框架中打开已有 Models 分区。消息任务会打开已有 Plugins 分区并选中其中的 `im` 子页；Codex 任务则打开“外部工具”分区，复用该页面的连接开关。“完成此项”会为对应任务显示绿色勾并返回总览；“返回步骤”则不会宣称任务已完成。每项任务以及整个流程都允许跳过。三项任务都有结果后，向导会根据结果展示“配置成功”欢迎页，或说明跳过项之后仍可在设置中补齐的“已准备就绪”页面。
