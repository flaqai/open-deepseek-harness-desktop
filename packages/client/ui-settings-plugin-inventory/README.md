...
In packaged desktop runs, the package also contributes a `shell.overlay` progress card for the deferred Better Sidebar archive. The card starts only the exact allowlisted package, reports coarse verify/extract/configure milestones, and remains non-blocking when hidden. Completion offers the narrow desktop restart capability; failure offers retry and the fixed Harness log. If the durable seed marker already exists—including after a later uninstall—the card stays absent and does not restore the plugin.
