...
使用**插件列表**可按稳定条目 id、启用状态、来源、运行状态与禁用条件检查 Agent 预设及全局插件。诊断与恢复界面会解释受控的 Profile 操作。探索插件使用市场目录与当前 Profile 状态完成安装，不维护重复排行。外部工具可安装官方子 Agent Provider 或社区 WorkBuddy 连接插件。安装时显示 pnpm 进度，可打开经过敏感信息清理的实时输出，并提供明确的暂停与停止操作，不会替换失败诊断。
...
打开**外部工具**即可安装受支持的 Codex、Claude Code Provider，或社区维护的 WorkBuddy 连接插件。WorkBuddy 会联网下载经过审核的 `dsh-workbuddy-connect@0.5.0`，并要求本机 WorkBuddy 或 WorkBuddy AI 桌面应用已经登录；该插件不随客户端预置。安装运行时，进度操作分为**暂停**和**停止**：暂停会安全结束当前包管理事务，随后提供**继续下载**，以相同请求重新开始并复用 pnpm 缓存；停止会保留明确的已停止状态，并允许重新下载。安装获得 id 后，**查看下载进度**按钮启用，并在当前设置页面生命周期内于成功、失败、暂停或停止后继续可用。关闭终端本身不会取消操作。用户向上滚动离开末尾时，界面暂停自动跟随；较早的受限容量输出被截断时会明确说明。
...
外部工具终端使用上一次返回的不透明字节偏移轮询 `getInstallOutput`，不会在每次状态快照中重复传输完整日志。视觉百分比只来自 `installProgress.percent`：解析确定总数后，已取得软件包用于下载进度，已导入软件包用于安装进度，其他阶段仍是不定进度。暂停和停止只提交 Host 签发的安装 id；渲染层不能发送 PID 或选择其他进程。官方 Provider 请求使用桌面兼容版本清单；WorkBuddy 卡片只把已签名的客户端操作映射到经过审核的社区包规格，并继续走同一个受控联网安装器。
