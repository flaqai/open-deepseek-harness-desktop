...
describe('ui-theme host', () => {
  it('registers, validates, and disposes the durable theme namespace with its fiber', async () => {
    ...
    await ctx.settings.update(ns, { preference: 'inspiration-collage' })
    expect(ctx.settings.get(ns)).toEqual({ preference: 'inspiration-collage' })
  ...
  it('answers each collection with the current durable preference until disposal', async () => {
    ...
    await ctx.settings.update(settingsNamespace(THEME_SETTINGS_NAMESPACE), { preference: 'inspiration-collage' })
    expect(scriptText(collect(ctx)[0])).toContain('const preference = "inspiration-collage"')
    await fiber.dispose()
