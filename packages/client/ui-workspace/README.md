---
description: "Shared Workspace browser, archived-session manager, and picker plugin for the dsh web client: grouped or flat session rows, add/rename/reorder, search, fork, archive/restore, and the directory-flow picking hole."
...
This package lets users browse grouped or flat Session lists, choose a Workspace for a new Session, and manage Workspaces and Sessions through add, rename, reorder, search, fork, archive, restore, and Workspace deletion. Pending interactions appear as warning dots, active scheduled tasks as alarm markers, and subagent-origin Sessions remain hidden. Canonically distinct folder paths remain separate Workspaces. Adding a Workspace requires a composed directory picker; without one, the add action is unavailable.
...
- **No permanent Session deletion** — the archive manager restores retained Sessions but does not erase their durable history; Workspace registration deletion also does not delete Sessions.
