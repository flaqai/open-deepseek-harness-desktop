...
function renderTab(
  ...
  bodies: SlotBodies = {}, extras: ActionBodies | Record<string, ConfigPageForm> = {},
) {
  const actionBodies: ActionBodies = Object.values(extras).every(value => typeof value === 'function') ? extras as ActionBodies : {}
  const forms: Record<string, ConfigPageForm> = actionBodies === extras ? {} : extras as Record<string, ConfigPageForm>
  ...
  const props: PluginManagerPageProps = {
    ...
    renderSlot: (name, owner, opts) => {
      ...
      if (name === 'plugins.bundle.action') return actionBodies[key]?.((owner as { enabled?: boolean }).enabled === true) ?? null
...
describe('PluginManagerPage', () => {
  ...
  it('renders a bundle-owned action beside the switch on its card and detail page', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: en.openDetail.replace('{name}', '@deepseek-ai/dsh-experimental-agent-team-web-profile') }))
    expect(screen.getByRole('button', { name: 'Use now' })).toBeTruthy()
    expect(action).toHaveBeenLastCalledWith(true)
  })
