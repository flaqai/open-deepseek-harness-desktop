...
describe('ApprovalPanel', () => {
  ...
  it('leaves disclosure Enter to its native toggle without approving', () => {
    const pending = new PendingApproval(id('s1'), { toolName: 'bash', callId: 'call-1' as ToolCallId })
    render(<ApprovalPanel {...panelProps(pending, () => <code>pnpm test</code>)} />)
    const summary = screen.getByText('Show command details')
    summary.focus()
