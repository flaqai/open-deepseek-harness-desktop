...
该文档只存放凭据，因此任何偏离都是拒绝，而不是跳过某个条目——被静默忽略的键读起来就是「我存进去的密钥没有生效」。非 mapping 的根、非 POSIX 标识符的键、非字符串值、空字符串、重复键以及格式错误的 YAML 全部失败：启动时明确报错，运行期热重载则告警并保留最后可用快照。格式没有包装层；当前版本不会写入 `version` 字段。为兼容早期桌面预览版，仅两种已知遗留格式会被读取：顶层、非负整数形式的 `version`，以及仅由该 `version` 与 `refs` 映射组成的 v1 包装层。两者都不会作为凭据暴露，并会在下一次受支持的凭据写入时移除或展平；其他非字符串条目仍会失败。
