...
The document holds credentials only, so every deviation is a rejection rather than a skipped entry — a silently ignored key would read as "the secret I stored has no effect". A non-mapping root, a key that is not a POSIX identifier, a non-string value, an empty string, a duplicate key, and malformed YAML all fail: loud at boot, and warn-and-keep-the-last-good-snapshot on a live reload. The format has no wrapper level, and current builds never write a `version` field. For compatibility with early desktop previews, only exact known shapes are read: a top-level non-negative integer `version`; a `version` plus `refs` wrapper (the wrapper version may be `1`, `"1"`, or `"v1"`); and a sole `refs` wrapper. Unknown sibling fields are never skipped or guessed. Neither wrapper metadata nor `refs` becomes a credential, and a supported credential write removes or flattens it into the canonical mapping; every other non-string entry still fails.
