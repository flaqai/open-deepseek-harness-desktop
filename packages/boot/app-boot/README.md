...
The Profile preflight reconciles dependency identity and quarantine state before composition. If an older or interrupted uninstall removed an inactive plugin and its durable quarantine but left lockfile, repair-report, or diagnostic references, inspection reports `profile.quarantine-removal-residue`; repair removes only those stale references and any incomplete package directory, so the absent plugin is not quarantined again. When a later successful installation restores the dependency, ordered Bundle, and package manifest and the current Loader proves that package root active, Host inventory removes the obsolete quarantine metadata without deleting the active package.

Before executing an active external bundle, preflight also reads a fixed package-root `compatibility.json`. A valid schema-v1 document lists exact supported Harness versions in `supportedHosts`; when that list excludes the running Harness version, the plugin is quarantined as `incompatible-host-version` and Diagnostics shows the current, supported, and optional recommended versions before offering market update discovery. Missing, malformed, oversized, symlinked, or unknown-schema declarations remain unknown and do not block activation. Broad `peerDependencies` are not treated as compatibility evidence.

For each uniquely owned external Loader row, preflight resolves the declared module without executing it and checks the entry file's static bare imports. An unavailable Loader module is quarantined as `loader-module-unresolvable`; an available Loader module with an unavailable imported dependency is quarantined as `loader-dependency-unavailable`, with the outer Bundle, Loader entry, importer, and missing package retained in the diagnostic. Runtime applies the same unique-ownership boundary when Node proves that an installed dependency lacks a named export expected by the plugin, retaining the dependency and export names before quarantining the incompatible plugin. User-targeted or ambiguous rows are never automatically removed. Diagnostic mode also redirects the settings provider to an app-maintained empty document, so invalid user settings remain untouched and cannot prevent the Diagnostics UI from loading. This recovery-only process does not turn a failed normal Profile startup into an application startup success.

Advisory source inspection flags an active external Bundle whose shipped JavaScript both subscribes to `agent/pre-step` and directly assigns a message field. The event publishes deeply frozen input, so an actual read-only-property failure is classified as `profile.immutable-agent-input-mutation`; source-only evidence names the direct Bundle but never executes, changes, or quarantines it.
...
- **Bootable plugin points retain their predecessor** — marking a new successfully started state demotes the previous bootable point to an automatic snapshot instead of deleting it. Content-identical automatic points deduplicate, while the current bootable point remains outside automatic-count pruning.
