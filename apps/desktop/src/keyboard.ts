...
type DesktopBrowserLeaseId = string
 ...
 * @param getRenderer - Harness content renderer, which differs from the window's titlebar renderer on Windows and Linux.
 * @param isTrustedPage - Exact selected local or NAS Runtime origin check.
...
export function installDesktopShortcuts(
  getWindow: () => BrowserWindow | undefined, getRenderer: () => WebContents | undefined,
  isTrustedPage: (url: string) => boolean, userData: string, platform: ShortcutPlatform, updateMenu: () => void,
  ...
  const sendMenuClose = (): void => {
    ...
    if (window === undefined || window.isDestroyed() || !window.isFocused() || !window.isEnabled()
      || renderer === undefined || renderer.isDestroyed() || revision === undefined || recording
      || overlayInput(window).blocked) return
    renderer.send(DESKTOP_IPC.shortcutsInput, { kind: 'menu', commandId: 'page.close', revision })
  }
  ...
  const publish = (snapshot: ShortcutConfigSnapshot): void => {
    ...
    const window = getWindow()
    const renderer = getRenderer()
    if (window !== undefined && !window.isDestroyed() && renderer !== undefined && !renderer.isDestroyed()
      && isTrustedPage(renderer.mainFrame.url)) {
      renderer.send(DESKTOP_IPC.shortcutsChanged, snapshot)
    }
  }
  ...
  const assertSender = (event: IpcMainInvokeEvent): BrowserWindow => {
    ...
    if (window === undefined || window.isDestroyed() || renderer === undefined || renderer.isDestroyed()
      ...
      || !isTrustedPage(event.senderFrame.url)) throw new Error('desktop shortcuts: rejected sender')
    return window
  }
  ...
  ipcMain.handle(DESKTOP_IPC.shortcutsRecording, (event, active: unknown) => {
    ...
    recording = active
    getRenderer()?.setIgnoreMenuShortcuts(active)
  })
  ...
  function attachInput(window: BrowserWindow, contents: WebContents, guestName?: DesktopBrowserLeaseId): () => void {
    ...
    const beforeInput = (event: Electron.Event, input: Input): void => {
      ...
      getRenderer()?.send(DESKTOP_IPC.shortcutsInput, { kind: guestName === undefined ? main ? 'keyboard' : 'iframe' : 'webview',
  ...
  return {
    ...
    attach(window) {
      const renderer = getRenderer()
      if (renderer !== undefined && !renderer.isDestroyed()) attachInput(window, renderer)
    },
