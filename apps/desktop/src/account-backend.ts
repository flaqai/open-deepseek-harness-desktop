...
import type { AccountClientMetadata, AccountView, SignInAttemptId } from './welcome-account-types.ts'

const REMOTE_STREAM_MUX_PATH = '/api/remote.mux'

function parseRemoteStreamServerMessage(text: string): { streamId: string; type: 'item' | 'end' | 'error'; value?: unknown } {
  const frame: unknown = JSON.parse(text)
  if (typeof frame !== 'object' || frame === null || Array.isArray(frame)
    || !('streamId' in frame) || typeof frame.streamId !== 'string'
    || !('type' in frame) || !['item', 'end', 'error'].includes(String(frame.type))) {
    throw new Error('desktop account: invalid stream frame')
  }
  return frame as { streamId: string; type: 'item' | 'end' | 'error'; value?: unknown }
}
