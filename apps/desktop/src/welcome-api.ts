...
import type { AccountView, SignInAttemptId } from './welcome-account-types.ts'
...
export interface WelcomeAuthentication {
  ...
  readonly wasPresented?: boolean
}

/**
 * Decide whether a data home's first startup requires the welcome entry.
 * @param authentication - current account and independently stored API-key facts.
 * @returns true only before the first presentation when neither authentication route is configured.
 */
export function needsWelcome(authentication: WelcomeAuthentication): boolean {
  return !authentication.wasPresented && !authentication.loggedIn && !authentication.hasApiKey
}
