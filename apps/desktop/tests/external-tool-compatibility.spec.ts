...
import manifestJson from '../external-tools/manifests/0.1.5-rc.2.3/external-tools-compatibility.v2.json'
...
function bundleBytes(
  ...
  subjectName = 'external-tools-compatibility-0.1.5-rc.2.3.v2.json',
): Uint8Array {
  const statement = {
...
describe('external tool compatibility', () => {
  ...
  it('accepts a signed remote manifest whose attested digest matches', async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(1,
      'https://flaqai.github.io/open-deepseek-harness-desktop/metadata/external-tools/v2/external-tools-compatibility-0.1.5-rc.2.3.v2.json',
    ...
    expect(await readFile(join(cacheDirectory, 'external-tools-compatibility-0.1.5-rc.2.3.v2.json'))).toEqual(Buffer.from(manifest))
  })
