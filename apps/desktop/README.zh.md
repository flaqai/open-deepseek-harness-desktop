...
RC.2 快捷键编辑器通过原子写入将本机按键设置保存在 Electron 的 `userData/keybindings.json`。预加载脚本只向当前选中的 Harness 渲染进程提供快捷键读取、编辑、录制、输入和关窗操作；IPC 会拒绝标题栏、其他窗口以及不属于当前本机或 NAS 运行端的来源。Windows 与 Linux 的原生按键送入 Harness WebContentsView，而不是单独的标题栏渲染进程。快捷键设置不会修改 `DSH_HOME` 或 NAS Profile。
...
跨电脑迁移时，在源电脑打开“设置 → 插件恢复”并选择原配置目录。选中要导出的已安装插件，再填写目标电脑的操作系统、CPU 架构和精确系统 release，而不是桌面客户端版本。源电脑可以联网获取 registry 的原始 `.tgz` 归档与传递依赖；也可以复用源 home 内已经过验证的归档。将生成的单个 `.tgz` 迁移包与另行复制的原配置目录一起带到目标电脑。迁移包不包含会话、凭据或正在使用的 `node_modules` 目录。

目标电脑的导入页会显示系统、CPU 架构和精确系统 release。将这些显示值填入源电脑的导出表单。

在目标电脑选择导入官方或社区配置，选择“离线迁移插件”，并在选择独立数据目录前选取迁移包。复制后的配置保留插件恢复清单，迁移包会暂存于新 home。在“设置 → 插件恢复”确认列出的包名和版本后，无需下载插件即可恢复。桌面端检查迁移包的条目与哈希，拒绝操作系统系列、架构或精确系统 release 不匹配的包，先在目标电脑离线验证依赖解析，再只把仍待恢复的条目安装到受管候选目录。激活失败会恢复原有受管依赖和恢复状态。被排除的自定义、Git、带凭据、缺失或外部工具来源不会被静默替换，需要导入后单独处理。构建包仍可能需要用户明确授权；归档有效也不保证插件能够激活。
...
原生模型引导页相对桌面模块加载 `renderer/welcome.html`，不依赖 Electron 开发模式下的 `appPath`。脚本和字体构建到 `lib/welcome/`；桌面安装包同时包含这两个目录。引导窗口沿用主工作区窗口的位置和大小，最小为 960 × 640。每个本地 `DSH_HOME` 都会在首次成功加载后、窗口显示前记录展示状态；即使跳过、关闭，或仍未登录且未填写 API Key，后续启动也不会再次自动弹出；切换数据目录则有独立的首次展示。
