...
import { ensurePackagedPrebuiltProfile, packagedPrebuiltProfileArchiveRoot } from '../lib/packaged-runtime.js'
...
const extraction = await mkdtemp(join(tmpdir(), 'dsh packaged profile source '))
try {
  const archiveRoot = packagedPrebuiltProfileArchiveRoot(process.platform, process.arch)
  const source = await ensurePackagedPrebuiltProfile({
    archivePath: join(resources, 'prebuilt-profile.tar'),
    checksumPath: join(resources, 'prebuilt-profile.tar.sha256'),
    destination: join(extraction, archiveRoot),
    archiveRoot,
  })
