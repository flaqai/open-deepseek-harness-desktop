...
test('collects stable metadata without file contents or source paths', async () => {
  ...
  assert.equal(first.files.some(entry => entry.label === 'native-smoke-entry'), false)
  ...
  assert.equal(first.smoke.journal.phases.find(phase => phase.name === 'native-entry')?.outcome, 'passed')
