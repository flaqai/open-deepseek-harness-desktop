...
Packaged releases carry pinned, integrity-checked archives for `dshmarket@1.19.0`, `@xmanrui/dsh-im@1.0.2`, `dsh-skill-picker@0.2.0`, `dsh-font@1.1.0`, `dsh-pocket@1.12.3`, and `dsh-better-sidebar@0.15.2`, plus the official `@deepseek-ai/dsh-subagent-codex@0.1.0-rc.8` connector with `@openai/codex@0.147.0` and only the target platform payload. Startup seeds the first five plugins; Better Sidebar installs only from its explicit discovery action, and Codex installs only from the explicit External Tools action. An online exact registry or pinned Git install is preferred so community plugins retain an updateable dependency identity; the checked archive is the offline fallback. Durable seed markers survive a later uninstall, so startup does not restore a removed plugin, while an explicit manual action can install it again. Packaging uses only these checked inputs and never copies plugins from the developer machine's Web profile.
