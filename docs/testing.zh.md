...
- **真实 API e2e**（`pnpm run test:e2e`）：显式调用线上 provider 的本地测试；各套件缺少自己的 key（`DEEPSEEK_API_KEY`、`EXA_API_KEY`、`PERPLEXITY_API_KEY` 等）时自动跳过，使仓库 CI 保持 keyless（[fork CI 信号 Agent Note](../.agents/notes/implemented/testing/2026-08-23-fork-ci-signal-scope.zh.md)）。
