...
- **Real-API e2e** (`pnpm run test:e2e`): explicit local tests against live providers; each suite self-skips without its own key (`DEEPSEEK_API_KEY`, `EXA_API_KEY`, `PERPLEXITY_API_KEY`, …), so repository CI stays keyless ([fork CI signal Agent Note](../.agents/notes/implemented/testing/2026-08-23-fork-ci-signal-scope.md)).
