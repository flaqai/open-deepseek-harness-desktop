...
/** Community desktop workspace packages that never enter the official npm family. */
const communityPrivatePackageDirectories = new Set([
  'apps/desktop',
...
const packageFileExtras: Readonly<Record<string, readonly string[]>> = {
  ...
  // Community translations are redistributed with their source attribution.
  '@deepseek-ai/dsh-client-ui-desktop-shell': ['COMMUNITY_TRANSLATION_LICENSES.txt'],
  ...
  '@deepseek-ai/dsh-subprocess-local': [
    'lib/runner.js',
    'lib/process-control.js',
    'lib/process-inspector-*.js',
    'lib/windows-job-*.js',
    'lib/runner-*.js',
    'lib/output.js',
...
export function checkWorkspaceManifest({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  const label = manifest.name ?? dir
  // The Electron application has an independent community release train;
  // source packages still follow the upstream workspace version.
  const familyVersionError = dir === desktopApplicationDirectory
    ? undefined
  ...
  } else if (isReleaseMemberDirectory(dir) && !communityPrivatePackageDirectories.has(dir)) {
  ...
  if (dir.startsWith('apps/') && dir !== desktopApplicationDirectory && manifest.name?.startsWith('@deepseek-ai/')
    && !communityPrivatePackageDirectories.has(dir)) {
