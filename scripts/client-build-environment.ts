...
/** Public client environment embedded in community desktop artifacts. */
const COMMUNITY_DESKTOP_CLIENT_BUILD_ENVIRONMENT = {
...
export function resolveClientBuildEnvironment(
  ...
  if (profile === 'community-desktop') {
    const commitHash = environment[CLIENT_COMMIT_HASH_VARIABLE]
    if (commitHash === undefined) {
      throw new Error(`${CLIENT_COMMIT_HASH_VARIABLE} is required for the community desktop client build profile`)
    }
    return { DSH_CLIENT_COMMIT_HASH: commitHash, ...COMMUNITY_DESKTOP_CLIENT_BUILD_ENVIRONMENT }
  }
  throw new Error(
    `unknown client build profile ${JSON.stringify(profile)}; expected "official" or "community-desktop"`,
  )
}
