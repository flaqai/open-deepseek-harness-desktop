...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-brand-community-desktop': { kind: 'none', reason: 'Community desktop presentation occupants; registers nothing model-facing.' },
  ...
  'packages/client/ui-selection-actions': { kind: 'none', reason: 'Browser-side draft controls; no selected text reaches model context until the human submits the resulting draft.' },
  ...
  'packages/client/ui-desktop-shell': { kind: 'none', reason: 'Electron-only desktop preferences and Release links; registers nothing model-facing.' },
  ...
  'packages/nas/nas-protocol': { kind: 'none', reason: 'NAS wire documents and validation never enter model requests.' },
