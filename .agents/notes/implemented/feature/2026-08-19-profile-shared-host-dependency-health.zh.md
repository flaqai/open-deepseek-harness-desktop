...
共享 Host 依赖健康是默认的 profile 启动能力，不是可选插件。受保护集合包含 `@deepseek-ai/cordis`、`@deepseek-ai/dsh-attachment`、`@deepseek-ai/dsh-llm`、`@deepseek-ai/dsh-scope`、`@deepseek-ai/dsh-system-prompt` 与 `@deepseek-ai/dsh-tools`。作用域包需要保护，因为不同物理副本使用不同 symbol，会令 Agent 作用域跨副本失效。
