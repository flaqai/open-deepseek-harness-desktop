# Agent Note: 桌面诊断演练中心

Status: implemented

[English](2026-08-28-desktop-diagnostics-lab.md) | 中文

## 问题

正式 Profile Doctor 能保护启动，但支持人员仍需要一种可重复的方法，展示依赖冲突、失管 Bundle、Loader 故障和构建拦截是怎样被发现与处理的。短暂的压力测试不适合这种演示：固定样本成功一次以后，重复相同场景并不会增加多少证据；自动清理还会在用户查看常规诊断统计和隔离操作之前抹掉现场。

演练仍必须安全。渲染层不能因此获得任意包管理器、路径、脚本或文件系统权限；主动保留的演练现场也不能与启动前应自动回滚的中断写入混为一谈。

## 决策

桌面宿主通过窄范围 Electron Bridge 提供版本化诊断演练中心。目录包含十六个固定场景标识和固定样本字节：兼容与不兼容 Host 影子副本、孤立 Bundle、旧版隔离卸载残留、scoped 根包与 unscoped Loader 包名不匹配、入口可解析但内部依赖缺失的 Loader、已安装 Host 依赖缺少插件要求的 API 导出、损坏的设置文档、安装包内 `dsh-font` 客户端不兼容、缺失模块、无效 Patch、外部 Bundle 复用安装方 Loader ID、生命周期失败、构建许可被阻止、修复中断，以及有界的启动操作超时。渲染层只能选择这些标识，以及隔离沙箱或需明确确认的当前 Profile。默认不选中任何场景。

每个选中场景只注入一次。成功运行进入持久的 `active` 阶段，不再自动清理。本次运行的文件、包管理器状态、修复处置和隔离记录会一直保留，直到用户点击**全部恢复**。报告保留预期与实际产品错误码、修复处置、耗时和经过限长脱敏的诊断。

默认沙箱会在 Electron `userData` 下使用本次运行专属的 home。它会经过正式 Doctor 边界并保留运行时，但不会影响当前 Profile 顶部统计。这是刻意的边界：沙箱证据显示在演练卡片与报告里，不能伪装成用户真实 Profile 的问题。

当前 Profile 模式只开放九种经过真实产品链路验证的场景：兼容 Host 影子副本、不兼容 Host 依赖、孤立 Bundle、旧版隔离卸载残留、scoped Loader 包名不匹配、Loader 内部依赖缺失、Loader 依赖缺少导出、损坏的 `settings.yaml`，以及安装包内 `dsh-font 1.1.0` 的客户端不兼容。写入前 Harness 会暂停，桌面宿主会备份白名单内的 Profile 清单、Workspace 策略、锁文件、Patch、用户设置、安全模式设置、隔离记录和健康报告。Host 影子样本只安装在自己的 `@dsh-diagnostic-lab/*` 命名空间根目录下，绝不替换 Profile 全局 Host override，也不修改 `nodeLinker`。前四种样本仍通过正式 `dsh plugin --profile web doctor --repair` 链路完成真实的依赖统一、隔离或状态收敛。隔离卸载演练会重建旧卸载器留下的精确不一致形态：插件、活动 manifest 条目与持久隔离已经消失，但受限的修复报告、诊断报告和 lockfile 引用仍然存在。正式 Doctor 必须报告 `profile.quarantine-removal-residue`，只清理这些派生状态，并保留其他无关 incident。`dsh-font` 被固定为完整性校验的 `diagnostic` 资源，普通启动与手动预装流程都不会安装；只有演练中心会通过 `dsh plugin add` 临时装入，再恢复真实浏览器，等待客户端 Loader 恢复链路写入 `profile.module-resolution`、撤下 Bundle 并隔离插件。因此常规诊断看到的就是保护真实用户启动的同一种记录。

Scoped 名称场景会通过普通插件 CLI 安装经过完整性固定的 `@dsh-diagnostic-lab/scoped-loader-mismatch@1.0.0` 资源。其根包使用 scoped 名称，而 Bundle Patch 故意指向不存在的 unscoped Loader 模块。安装后 Profile 预检必须把最终 entry 唯一归属到这个直接根包，立即以 `loader-module-unresolvable` 隔离，并保留真实的诊断计数与解决操作。沙箱模式会在本次运行专属 DSH home 中执行同一包，因此其隔离不会改变活动 Profile。

内部依赖缺失场景同样安装经过完整性固定的 `@dsh-diagnostic-lab/loader-dependency-unavailable@1.0.0`，但它的 Loader entry 本身可以解析，随后静态导入一个不存在的内部 Host 模块。相同的预检必须报告 `loader.dependency-unavailable`，把责任归到聚合根插件，并以 `loader-dependency-unavailable` 隔离。损坏设置场景会向所选 home 写入重复键并保留精确字节；当前 Profile 模式会恢复真实 Harness，直到安全模式报告记录 `config.settings-invalid` 和 `skippedUserSettings: true`，再验证安装方维护的空设置文件没有替换损坏的用户文档。运行时恢复场景最后依次执行缺少导出、`dsh-font` 和损坏设置，避免持续保留的设置错误遮蔽任一 Loader 演练。“全部恢复”会还原逐字节一致的原设置，并删除演练前不存在的安全模式设置文件。

缺少导出场景只在当前 Profile 中安装经过完整性固定的 `@dsh-diagnostic-lab/loader-export-unavailable@1.0.0`。它的 Loader 模块会从已安装的 settings Host 导入一个故意不存在的命名导出。演练中心恢复真实 Harness，并要求运行时恢复链保留依赖名与导出名，把最深层 Loader entry 唯一归属到外部根插件，以 `loader-dependency-unavailable` 隔离该根插件，而且不能先依赖安全模式 UI。所有不重启的场景结束后才运行这些运行时恢复演练，并把损坏设置放在最后，避免一个持续保留的启动故障遮蔽其他演练。

启动操作超时场景只允许在沙箱运行。它会调用桌面端拥有的假 CLI；该程序写入私有 marker 后持续运行。正式的 `DesktopOperationSupervisor` 会应用缩短的演练期限、终止进程树、返回结构化超时结果、删除 marker 以模拟事务回滚，并证明执行器可以继续下一步骤。该场景不会打开或修改活动 Profile，也不会真的等待生产环境的完整超时时长。

恢复日志区分四种状态。`injecting` 与 `restoring` 表示未完成事务，进程中断后必须在加载 Profile 插件前自动回滚；`active` 表示用户主动保留的演练，应用重启后应重新连接到界面；`clean` 表示全部恢复已经完成。这样既不会让崩溃恢复误删演示现场，也不会放行只写了一半的修改。

“全部恢复”是一项明确事务。当前 Profile 模式会再次暂停 Harness，恢复已备份的受管文件，只删除演练命名空间、固定样本源目录、命名空间内的已安装根和可归因于本次运行的 pnpm 链接，再执行离线强制依赖重建。随后重新落回并校验备份文件的精确哈希，确认没有本次运行的路径或符号链接残留，并通过最终只读 Doctor 后才把恢复日志标为 clean。只有所有检查都成功，Harness 才会恢复；恢复失败时保留可重试入口、禁止再次演练并维持 Harness 停止。下次启动会把失败恢复交给界面，并继续进入受监管的安全模式，而不是让桌面应用直接退出；安全模式的裸模块导入使用有效的 `file:` 基准 URL。沙箱模式只删除本次运行的 runtime。取消或断言失败会自动回滚，因为此时尚未建立有效的演练现场。

## 曾考虑的替代方案

**保留快速、标准和耐久档。** 否决，因为不可变样本重复一轮、三轮或十轮，在首轮成功以后并不会改善面向用户的诊断，反而让演练看起来像无关的基准测试。

**复检后始终自动清理。** 否决，因为用户无法查看隔离计数、尝试常规修复或卸载，也无法截取真实诊断现场。

**把沙箱问题投影到当前 Profile 顶部统计。** 否决，因为这会让合成的沙箱状态看起来像真实 Profile 故障。只有当前 Profile 模式可以改变常规统计。

**让所有样本都修改当前 Profile。** 否决，因为损坏 Patch、生命周期故障和受控中断属于分类器与恢复契约样本，不是由包管理器拥有的 Profile 插件；这些场景继续只在沙箱开放。

**允许渲染层传入任意包或脚本。** 否决，因为这会把诊断界面变成通用代码执行 Bridge。场景目录、样本字节、包名和目标路径仍完全由桌面主进程持有。

## 结果

演练中心现在展示的是持久现场，而不是压力循环。用户可以在当前 Profile 模式查看真实隔离计数，对这些插件使用现有诊断操作，重启应用后继续保留现场，并通过“全部恢复”回到演练前的受管配置。沙箱始终完全隔离，因此不会污染顶部统计。

活动任务由 Electron 主进程和 schema 2 恢复日志/报告持有。`current` Bridge 操作使重载后的渲染页面能够重新连接；根级 `shell.overlay` 卡片显示注入进度、现场保留状态、通过/失败数量和“全部恢复”。隐藏卡片只改变展示。

聚焦测试固定了单次注入、默认空选择、样本不会写入 Profile 全局 Host override、沙箱持久状态、真实 Profile 隔离统计、旧版卸载残留的写入与有界清理、强制依赖恢复、clean 日志下的残留恢复、失败关闭的重启行为、有效的安全模式模块 URL、重启后重新连接、启动进程树超时与模拟回滚、取消回滚、报告脱敏、受限 Preload Bridge、设置页呈现和类型安全的本地化。本桌面能力的验证通道不包含浏览器回放或 Playwright。
