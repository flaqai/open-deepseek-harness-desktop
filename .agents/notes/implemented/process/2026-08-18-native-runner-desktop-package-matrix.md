...
`apps/desktop/scripts/prepare-unix-runtime.mjs` receives the same explicit darwin-arm64, darwin-x64, or linux-x64 target as the Codex bundle and electron-builder invocation. It downloads the matching pinned Node 24.11.1 archive after verifying the official SHA-256 value, deploys the production Harness closure, verifies the target's Koffi, Sharp, and require-builtin native packages, embeds pnpm 11.7.0, and writes a target-specific archive. Desktop and Web package scripts run from their owning app directories instead of selecting them through the repository workspace, so platform-specific workspace packages do not participate in unrelated app command discovery. Packaged macOS and Linux applications extract that archive into versioned user data and start Harness with its embedded Node and pnpm.
