...
Optional bundles are downloaded with the product and stay inactive until selected; the runtime isolation smokes still observe no experimental module in a default composition. Switching on an optional Web layer loads its client plugin through the live client module graph. The Agent Teams pair is the only current companion relationship the page coordinates; other optional bundles remain independent selections.
