...
桌面单元测试固定原生 Node 启动参数、内置包管理器环境、三次失败上限和明确重试。运行时准备会实际执行内置 Node、pnpm 和 Harness 就绪路径。Windows 工作流把最终 NSIS 产物安装到包含空格和中文字符的路径，验证资源布局，并要求已安装应用在上传产物前进入 Harness 就绪状态。
