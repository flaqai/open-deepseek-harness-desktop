...
The macOS File menu routes Close Page or Window through the same owner and displays the accepted single-key binding. Native closure requires the current configuration revision, a focused and enabled product window, and inactive shortcut recording. The existing window lifecycle applies without a shortcut-specific confirmation. In the community Desktop, window close follows the tray-or-quit preference; the quit path uses the ordinary task-aware confirmation.
...
Close behaves consistently across the shortcut and macOS File menu. The command adds no background Host lifetime or shortcut-specific confirmation; a community Desktop quit remains subject to the shared task-aware check. Desktop keyboard tests cover revision and focus guards; sidebar focus tests cover captured page identity and stale targets; startup tests cover the platform menu declarations.
