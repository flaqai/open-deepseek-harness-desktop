...
静态加载文档显示一条从左向右的确定进度条。桌面环境准备、打包运行时准备、Profile 兼容性检查或修复、各个预设插件的校验、解压与配置，以及 Harness 启动的主进程里程碑会驱动百分比和当前任务文本。Preload 会先订阅再读取当前快照，因此页面导航不会丢失更早的里程碑。每个运行步骤都会携带开始时间和截止时间；等待四十秒后，页面会显示当前操作、已等待时间、剩余期限、自动降级策略和日志入口。有界的可选步骤被跳过时，进度会进入明确的降级状态。只有收到规范就绪行后进度才达到 100%，随后宿主才导航到 Web GUI。
