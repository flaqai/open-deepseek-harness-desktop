...
macOS File 菜单通过同一个 owner 路由“关闭页面或窗口”，并显示已接受的单键绑定。原生关闭要求配置 revision 仍为当前值、产品窗口已聚焦且启用，并且没有在录制快捷键。现有窗口生命周期直接生效，不增加快捷键专用确认。社区 Desktop 的关窗动作遵循托盘或退出偏好；退出路径使用普通的任务感知确认。
...
快捷键与 macOS File 菜单的关闭行为一致。该命令不增加后台 Host 生命周期或快捷键专用确认；社区 Desktop 完整退出仍受共用的任务感知检查约束。Desktop 键盘测试覆盖 revision 和焦点保护；侧栏焦点测试覆盖捕获的页面身份和过期目标；启动测试覆盖各平台菜单声明。
